# FortiManager MCP — Personal Fork (ollieS1992)

> **This is a personal fork of [rstierli/fortimanager-mcp](https://github.com/rstierli/fortimanager-mcp).**
...
1. **Workspace auto-lock for `/pm/config/*` and `/pm/pkg/*` writes.**
   In workspace mode, upstream writes fail with a misleading `No permission for the resource` error because the ADOM isn't locked first. This fork wraps all writes (`add`/`set`/`update`/`delete`/`move`) in an async context manager that transparently locks the ADOM, performs the write, commits, and unlocks. Also fixes the username/password branch to pass `check_adom_workspace=False` so pyfmg's own (buggy) workspace pre-check doesn't reject writes. See `apply_workspace_fix.py`.

2. **Smart install scope reconciliation.**
   `install_package` was failing on packages whose actual scope (device + vdom) didn't match what the caller passed. This fork queries the package's real scope from FortiManager and auto-corrects vdom mismatches before calling `/securityconsole/install/package`. See `apply_install_smart.py`.

## Verified against

- FortiManager-VM 7.6.6 build3654
- FortiGate-70G running FortiOS 7.6.6
- Workspace mode: `normal`
- Auth: REST API Administrator (token-based)

## Status

Tested in a single home-lab ADOM (`root`). Not yet PR'd upstream. The `apply_*.py` scripts are idempotent and capture the exact transformations applied to `client.py` and `system_tools.py`.
